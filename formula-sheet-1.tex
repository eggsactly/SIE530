...
0 & otherwise
   \end{cases}$
\subsubsection{Broken Down Example}
Assume X and Y are independent.
Say $Z_1=X+Y$ and $Z_2=\frac{X}{X+Y}$\\
$$\begin{cases} Z_1 = X+Y\\Z_2=\frac{X}{X+Y} \end{cases}$$
Therefore
$$\begin{cases} X=Z_1Z_2\\Y=Z_1(1-Z_2) \end{cases}$$
$$|J|=
\begin{vmatrix} \frac{\partial X}{\partial Z_1} & \frac{\partial X}{\partial Z_2} \\
\frac{\partial Y}{\partial Z_1} & \frac{\partial Y}{\partial Z_2} \end{vmatrix}$$
$$=
\begin{vmatrix}Z_2 & Z_1 \\
1-Z_2 & -Z_1\end{vmatrix}
=|-Z_1Z_2-Z_1(1-Z_2)| = Z_1
$$
$$f_{Z_1Z_2}(Z_1, Z_2)=f_{XY}(X,Y)*|J|$$
...
\subsection{$HyperGeometric(N,M,K)$}
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pmf} & ${{{K \choose x} {{N-K} \choose {n-x}}}\over {N \choose n}}$; \\
...
\emph{notes} & N: sample size, n: selected items w/o replacement, D: items of interest in the population, x: number of points of interest in n with property D.
\end{tabularx}

\subsection{$Bernoulli(p)$}
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pmf} & $\textstyle p^x (1-p)^{1-x}$; \\
\emph{cdf} & $ \begin{cases}
    0 & \text{for }x<0 \\ 1 - p & \text{for }0\leq x<1 \\ 1 & \text{for }x\geq 1
    \end{cases}$ \\
\emph{mean and variance} & $EX=p$; $Var X =p(1-p)$ \\
\emph{mgf} & $1-p + pe^t \!$ \\
...
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pdf} & $f(x|\alpha,\beta)=\frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha)\Gamma{\beta}}x^{\alpha-1}(1-x)^{\beta-1}$ \\
\emph{mean and variance} & $EX=\frac{\alpha}{\alpha + \beta}$; $Var X =\frac{\alpha \beta}{(\alpha + \beta)^2 (\alpha + \beta + 1)}$ \\
\emph{mgf} & $1+\sum_{k=1}^\infty (\prod_{r=0}^{k-1}\frac{\alpha + r}{\alpha + \beta + r})\frac{t^k}{k!}$ \\
\emph{notes} & none.\\
\end{tabularx}

\subsection{$Binomial(n,p)$}
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pmf} & $\textstyle {n \choose x}\, p^x (1-p)^{n-x}$; \\
\emph{cdf} & $\textstyle I_{1-p}(n - x, 1 + x)$ \\
\emph{mean and variance} & $EX=np$; $Var X =np(1-p)$ \\
\emph{mgf} & $(1-p + pe^t)^n \!$ \\
\emph{notes} & Probablity for two mutually exclusive outcomes. p: probability of desired outcome, n: sample size, x: number of items with outcome.\\
\end{tabularx}

\subsection{$Poisson(\lambda)$}
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pmf} & $P(X=x|\lambda)=\frac{e^{-\lambda}\lambda^x}{x!}$; $x=0, 1, \dots$; $0 \leq \lambda <\infty$ \\
\emph{cdf} & $\frac{\Gamma(\lfloor k+1\rfloor, \lambda)}{\lfloor k\rfloor !}$ \\
\emph{mean and variance} & $EX=\lambda$; $Var X =\lambda$ \\
\emph{mgf} & $M_X(t)=e^{\lambda(e^t-1)}$ \\
\emph{notes} & Number of events occuring in a time period, $\lambda$: Average rate of occurance, x: number of times in the period.\\
\end{tabularx}

\subsection{$Chi-squared(p)$}
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pdf} & $f(x|p)=\frac{1}{\Gamma(p/2)2^{p/2}}x^{(p/2)-1}e^{-x/2}$; $0 \leq x < \infty$; $p=1, 2, \dots$ \\
\emph{cdf} & $\frac{1}{\Gamma\left(\frac{k}{2} \right)} \; \gamma\left(\frac{k}{2},\,\frac{x}{2}\right)$ \\
...
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pdf} & $f(x|\beta)=\frac{1}{\beta}e^{-x/\beta}$; $0 \leq x < \infty$; $\beta>0$ \\
\emph{cdf} & $1-e^{-\lambda x}$ \\
\emph{mean and variance} & $EX = \beta$; $Var X = \beta^2$ \\
\emph{mgf} & $M_X(t)= \frac{1}{1-\beta t}, \quad t < \frac{1}{\beta}$ \\
\end{tabularx}

\subsection{$Gamma(\alpha, \beta)$}
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pdf} & $f(x|\alpha, \beta)=\frac{1}{\Gamma(\alpha)\beta^{\alpha}} x^{\alpha - 1}e^{-x/\beta};$ $0 \leq x < \infty$; $\alpha\beta>0$ \\
\emph{cdf} & $\frac{1}{\Gamma(k)} \gamma\left(k,\, \frac{x}{\theta}\right)$ \\
...
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pdf} & $f(x|\mu, \sigma^2)=\frac{1}{\sqrt{2\pi \sigma^2}}e^{-(x-\mu)^2/(2\sigma^2)}$; $-\infty < x < \infty$; $-\infty < \mu < \infty$ \\
\emph{cdf} & $\tfrac12\left[1 + \operatorname{erf}\left( \frac{x-\mu}{\sigma\sqrt{2}}\right)\right]$\\
...
\begin{tabularx}{\textwidth/\NumCols}{ l X }
\emph{pdf} & $f(x|a, b)=\frac{1}{b-a}$, $a \leq x \leq b$ \\
\emph{cdf} & $\begin{cases}
                  0               & \text{for } x < a \\
