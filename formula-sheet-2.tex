...
Example: Calculate the Posterior Distribution (i.e. the Bayes estimator) of $\lambda$. $X_1\dots X_n$ iid on Poisson, $\lambda \sim Gamma(\alpha, \beta)$.
...
$$Bias(\hat{\theta})=E(\hat{\theta})-\theta$$
$$Var(\hat{\theta})=E{[\hat{\theta}-E(\hat{\theta})]^2}$$
Example: find the MSE of the estimators $\bar{X}$ and $S^2$ for the normal distribution in terms of $\mu$ and $\sigma$.
